...
for(file in file_list){
  ...
  df_aux <- df_aux %>% group_by(anoepi, epiweek, SG_UF) %>% summarise(n = n())
...
df <- df %>% group_by(anoepi, epiweek, SG_UF) %>% summarise(n = sum(n))
save(df, file = 'timeseries_dengue_2010_2023_SG.RData')
